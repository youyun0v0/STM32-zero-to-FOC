...
void PWM_Init(void)
{
    //1.配置时基单元，可以找Timer.c代码，不需要打开中断，配置NVIC
        ...
        TIM_TimeBaseStructure.TIM_Prescaler = 36 - 1; //预分频值PSC，现在是20KHz，超出了人耳分辨范围，电机旋转时就不会再听到类似蜂鸣器的叫声
    ...
    //2.初始化输出比较单元
        ...
        TIM_OC3Init(TIM2,&TIM_OCInitStructure); //由于我们使用的是TIM2的OC2通道，查引脚定义表可知其复用在PA1引脚上
    //3.初始化GPIOA
        RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);//打开GPIOA时钟（供电）
        GPIO_InitTypeDef GPIO_InitStructure;//引脚工作参数配置
        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;//选择GPIOA的A2引脚
