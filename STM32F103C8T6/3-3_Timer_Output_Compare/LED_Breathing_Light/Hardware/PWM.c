...
void PWM_Init(void)
{
    //1.配置时基单元，可以找Timer.c代码，不需要打开中断，配置NVIC
        ...
        TIM_TimeBaseStructure.TIM_Period = 100 - 1; //自动重载值ARR，分辨率1/(ARR+1)=1%
        TIM_TimeBaseStructure.TIM_Prescaler = 720 - 1; //预分频值PSC，定时器时钟频率 = 72MHz/(PSC+1)，设置为720-1，定时器时钟频率为100kHz,PWM频率为100k/(ARR+1)=1kHz
    ...
    //2.初始化输出比较单元
        ...
        TIM_OCInitStructure.TIM_Pulse=0;//设置CCR,CCR=50时，占空比=CCR/(1+ARR)=50%
        //剩下的结构体成员都是高级定时器才需要用的.由于只给部分成员赋值，为了避免不确定的问题，我们需要给结构体先赋一个初值，再修改我们关心的成员
        TIM_OC1Init(TIM2,&TIM_OCInitStructure); //由于我们使用的是TIM2的OC1通道，查引脚定义表可知其复用在PA0引脚上
    //3.初始化GPIOA
